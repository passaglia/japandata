...
def load_data(year, datalevel='prefecture'):
    assert (datalevel in ['prefecture', 'local','prefecturemean','designatedcity','capital'])
    ...
    cols = []
    if (year >= 2011) and (datalevel == 'local'): cols += ['code6digit']
    cols += ['prefecture']
    ...
    df = pd.read_excel(DATA_FOLDER+datalevel+'/'+filelabel+fileextension, skiprows=skiprows,header=None, names=cols, dtype=forced_coltypes)
...
def clean_data():
    ...
    for year in years:
        ...
        if year < 2011:
            ...
            def findCode(row):
                ...
                potentialcities = [city, city.replace('ケ', 'ヶ'), city.replace('ヶ','ケ'), city.replace('桧','檜'), city.replace('竜','龍'),city.replace('曾','曽')]
                try:
                    codefound = map_df.loc[(map_df['city'].isin(potentialcities) | map_df['county'].isin(potentialcities)) & (map_df['prefecture']==pref), 'code'].values[0]
                except IndexError:
                    codefound = alt_map_df.loc[(alt_map_df['city'].isin(potentialcities) | alt_map_df['county'].isin(potentialcities)) & (alt_map_df['prefecture']==pref), 'code'].values[0]
                return codefound
    ...
    designatedcity_array = xr.concat([df.to_xarray() for df in df_designatedcity_list], dim=xr.DataArray(years,dims='year'))
    capital_array = xr.concat([df.to_xarray() for df in df_capital_list], dim=xr.DataArray(years,dims='year'))
...
try:
    ...
    if not checkfordata():
        getdata()
    pref_ind_xr, prefmean_ind_xr, local_ind_xr, designatedcity_ind_xr, capital_ind_xr = clean_data()
